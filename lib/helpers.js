		...
		fs.readdir(dir,function(err,files){
			if (err){
				...
				fs.writeFile(writePath, allFilesJSON, function(e) {
					if (!e) {
						console.log('Helper: Writing json with files');
						callback(status);
					}else{ 
						console.log('Helper: Error writing json with files', e);
					};
				});
			};
		});
	 ...
	 xhrCall: function (url,callback) { 
		...
		request({
			...
			if(!error){
				...
				console.log('Helper: XHR Error',error);
				setTimeout(function(){console.log('Helper: Waiting for retry...')},10000);
			}
		});
	}
}