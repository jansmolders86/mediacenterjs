...
exports.server = function(port, macAddress, options, device) {
    ...
    var movieSession = null;
    app.post('/play', function (req, res) {
        console.log(req.body);
        var videoURL = req.body["Content-Location"];
        if (videoURL === null) {
            videoURL = "http://" + req.body.host + req.body.path
        }
        movieSession = device.playMovie(videoURL, {
    ...
    app.post('/scrub', function(req, res) {
        var seconds = req.query.position;
        movieSession.scrub(seconds);
        res.status(200);
        res.set('Content-Length', '0');
        res.send();
    });
    ...
    app.get('/scrub', function(req, res) {
        movieSession.getCurrentPosition(function(pos) {
            res.send(textParametersResponse({position: pos}));
        });
    })
    var advertisment;
...
function textParametersResponse(obj) {
    var str = "";
