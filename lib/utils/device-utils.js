        ...
        getLocalIp(req, function(localIP){
            ...
            getPlatform(req, function(devicePlatfrm){
                ...
                getDeviceStatus(deviceID, function(isAllowed){
                    db.query('INSERT OR REPLACE INTO devices VALUES(?,?,?,?)', [ deviceID, currentDate, isAllowed, "no" ]);
                });
            })
        })
...
exports.lockDevice = function(req,res,deviceID) {
    var currentDate  = getDateTime();
    db.query('INSERT OR REPLACE INTO devices VALUES(?,?,?,(SELECT live_active FROM devices WHERE device_id = ?))', [ deviceID, currentDate, 'no', device_id ]);
...
exports.unlockDevice = function(req,res,deviceID) {
    var currentDate  = getDateTime();
    db.query('INSERT OR REPLACE INTO devices VALUES(?,?,?,(SELECT live_active FROM devices WHERE device_id = ?))', [ deviceID, currentDate, 'yes', device_id ]);
...
function getDeviceStatus(deviceID, callback){
    db.query("CREATE TABLE IF NOT EXISTS devices (device_id text PRIMARY KEY, last_seen text, is_active text, live_active text)");
...
function setDeviceActive(deviceID, active) {
	db.query('UPDATE devices SET live_active = ? WHERE device_id = ?', [active, deviceID] ? "yes" : "no");
}

function getDeviceActive(deviceID, active, callback) {
	db.query('SELECT live_active FROM devices WHERE device_id = ?', [ device_id ], {
		live_active: Boolean,
	}, function(rows) {
		if (typeof rows !== undefined && rows.length > 0) {
			callback(rows[0].live_active == "yes" ? true : false);
		} else {
			callback(false);
		}
	});
