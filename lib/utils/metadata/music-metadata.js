...
db.query("CREATE TABLE IF NOT EXISTS tracks (title TEXT PRIMARY KEY, track TEXT, album TEXT, artist TEXT, year TEXT, filename TEXT)");
        ...
        (function (filename) {
            ...
            parser.on('metadata', function (result) {
                ...
                if(result){
                    ...
                    getMusicMetadata(title, track, album, artist, year, filename,  function(result){
                        setTimeout(function() {
                            process.exit(0);
                        },1000);
                    });
...
getMusicMetadata = function( title,track, album, artist, year, filename, callback){
    console.log('Gettings tracks and albums for: ',album);
    ...
    getTrackFromDB(title,function(result){
        ...
        var metadata =  [title,track, album, artist, year, filename];
