...
var SUPPORTED_FILETYPES = "mp3";  //Pipe seperated
var start = new Date();
var nrScanned = 0;
...
var setupParse = function(results) {
	...
	if (results && results.length > 0) {
        ...
        var stop = new Date();
		console.log("Done scanning, time taken:", UMS((stop - start) / 100, true));
		process.exit();
	}
};
	...
	parser.on('metadata', function(result) {
        if(result){
            ...
            getTrackFromDB(title,function(result){
                ...
                var trackMetadata =  [title,track, album, artist, year, genre, file, file];
