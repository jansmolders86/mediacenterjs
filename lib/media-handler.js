...
MediaHandler.prototype.updateMetadata = function (id, newData, callback) {
  ...
  this.list_entity.find(id).success(function(object) {
     object.set(newData);
     self.metadataFunc({ href: object.filePath, file: path.basename(object.filePath) }, callback);
  });
}
...
MediaHandler.prototype.search = function (req, res, findOptions) {
  findOptions = findOptions || {};
  if (!findOptions.where) {
      findOptions.where = {title: {like: '%' + req.query.q + '%'}};
  }
  this.list_entity.findAll(findOptions)
